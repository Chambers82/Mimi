...
# SSE Program
# Attack: SQL Inject (Low Hanging Fruit)
...
def get_abs_url(link):
    """
    check if the link is relative and prepend the protocol and host. filter
    unwanted links like mailto and links that do not go to our base host
    """
...
def spider(url):
    """
    check if we don't know the url
    spider to the url
    extract new links
...
def found_error(content):
    """
    try to find error mesg in html
    """
    got_error = False
