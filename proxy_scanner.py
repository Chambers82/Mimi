...
# SSE Program
# Attack: HTTP Proxy Scanner
# Date: February 1, 2016
# Description:  Scan for open proxies and test for outbound connectivity
...
#Commonly used proxy ports
proxy_ports = [3128, 8080, 8181, 8000, 9090, 1080, 80]

# URL to fetch
get_host = "www.google.com"
socket.setdefaulttimeout(3)

# get a list of ips from start / stop in
...
# Attempt connects to the proxy and fetch the url
def proxy_scan(ip):
...
if len(sys.argv) < 2:
    ...
    if sys.argv[1].find('-') > 0:
        ...
        proy_scan(sys.argv[1])

'''
The call to socket.socket(socket.AF_INET, socket.SOCK_STREAM)
